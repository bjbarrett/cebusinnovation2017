...
setwd("~/Dropbox/InnovationProject/Analysis/final analysis/group levels")
#setwd("~/Dropbox/InnovationProject/Analysis/final analysis/Final all ages > 20 scans")
c <- read.csv("PNASfinaldomainindividual2ALL.csv" , header=TRUE)
...
#pdf("Fig1.groupannualinnoest.pdf", width=11.4 , height=8 , units="cm" )
pdf("Fig1.groupannualinnoest.pdf", width= 11.4/2.54, height=5.3/2.54 )

par(mfrow = c(2,5))
par(cex = .6)
par(mar = c(0, 0, 0, 0), oma = c(3.5, 3.5, 0.25, 0.25) )
...
link2 <- link(m, n=1000 , data=d.pred)
pred.med <- apply(link2  , 2 , median)
dens(link2, col="white", main="" , ylim=c(0,0.9) , xlim=c(0,10) , xlab="" ,ylab="" , xaxt="n" ,zero.line = FALSE , cex.axis=0.75)
shade( density(link2) , lim= as.vector(HPDI(link2, prob=0.99999)) , col = col.alpha("orange", 0.75))
...
dens(link2, col="white", main="" , ylim=c(0,0.9) , xlim=c(0,10) , xlab="" ,ylab="", xaxt="n" , zero.line = FALSE , cex.axis=0.75)
shade( density(link2) , lim= as.vector(HPDI(link2, prob=0.99999)) , col = col.alpha("orange", 0.75))
axis(1, seq(from=0 , to=10 , by = 1), labels=FALSE , tck=-0.04)
axis(1, seq(from=0 , to=10 , by = 2), labels=c("",2,4,6,8,10) , tck=-0.08 ,  cex.axis=0.75)
...
abline(v=pred.med)
 title(grouplist[i], line = -1 , cex=.5 , bg="blue")
 axis(1, seq(from=0 , to=10 , by = 1), labels=FALSE , tck=-0.04, cex.axis=0.75)
 axis(1, seq(from=0 , to=10 , by = 2), labels=c("",2,4,6,8,10) , tck=-0.08 , cex.axis=0.75)

}

mtext("estimated annual innovation rate per group", outer =TRUE, cex = .8, side=1, line=2.25)
mtext("posterior density", outer =TRUE, cex = .8, side=2, line=2)
...
d$gd_index <- as.integer(as.factor(with(d, paste(MainGroup, domain, sep="_"))))

mD <- map2stan(
    alist(
    inno ~ dpois( lambda ),
    log(lambda) <- a + ad + ag + agd + logsampeff ,
    ad[domain_index] ~  dnorm(0, sigma_d),
    ag[group_index] ~ dnorm( 0 , sigma_g ),
    agd[gd_index] ~ dnorm( 0 , sigma_gd ),

	a ~ dnorm(0,3),
    c(sigma_g,sigma_d,sigma_gd) ~ dcauchy(0,2) 
),
data=list(
	inno = d$GDYsum,
	...
	gd_index = d$gd_index,
	logsampeff=log(d$sampeff/365)
	) , 
warmup=2000 , iter=4000 , chains=3 , cores=3 
)

save(d,dd,mD,m, file="groupinnorate.RData")
