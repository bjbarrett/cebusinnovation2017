...
d <- read.csv("~/data_inno_domain_git.csv" , header=TRUE) #direct to local directory

######BEGIN VARYING EFFECTS OF DOMAIN MODELS#######
mD <- map2stan(
    alist(
        ...
        al_id[mono_index] ~ dnorm(0, sigmal_id),
        ap_id[mono_index] ~ dnorm(0, sigmap_id),
        al_group[group_index] ~ dnorm(0, sigmal_group),
        ap_group[group_index] ~ dnorm(0, sigmap_group),
        al_dom[domain_index] ~ dnorm(0, sigmal_dom),
        ap_dom[domain_index] ~ dnorm(0, sigmap_dom),
        c(ap,al)~ dstudent(2,0,1),
        c(sigmap_id,sigmal_id,sigmap_group,sigmal_group,sigmal_dom,sigmap_dom) ~ dexp(1)
...
data=list(
    ...
    cores=3 , chains=3 , warmup=1500, iter=3000, WAIC=TRUE , types=list(adapt.delta=0.9)
)

#save(d,mD,mD.age,mD.soc,mD.rank,mD.male,mD.ageglobal,file="12Jandomainzeros.RData")
save(d,mD,file="13Jandomainzeros.RData")
...
mD.age <- map2stan(
    alist(
        ...
        c(al_id,bagel_id)[mono_index] ~ dmvnorm2(0, sigmal_id, Rho_l_id),
        c(ap_id,bagep_id)[mono_index] ~ dmvnorm2(0, sigmap_id, Rho_p_id),
        c(al_group,bagel_group)[group_index] ~ dmvnorm2(0, sigmal_group, Rho_l_group),
        c(ap_group,bagep_group)[group_index] ~ dmvnorm2(0, sigmap_group, Rho_p_group),
        c(al_dom,bagel_dom)[domain_index] ~ dmvnorm2(0, sigmal_dom, Rho_l_dom),
        c(ap_dom,bagep_dom)[domain_index] ~ dmvnorm2(0, sigmap_dom, Rho_p_dom),
        c(ap,al)~ dstudent(2,0,1),
        c(bagep,bagel)~ dnorm(0,1),
        c(sigmap_id,sigmal_id,sigmap_group,sigmal_group,sigmal_dom,sigmap_dom) ~ dexp(1),
        c(Rho_p_id, Rho_l_id,Rho_p_group, Rho_l_group , Rho_p_dom, Rho_l_dom) ~ dlkjcorr(3)
...
mD.soc <- map2stan(
    alist(
        ...
        c(al_id,bsocl_id)[mono_index] ~ dmvnorm2(0, sigmal_id, Rho_l_id),
        c(ap_id,bsocp_id)[mono_index] ~ dmvnorm2(0, sigmap_id, Rho_p_id),
        c(al_group,bsocl_group)[group_index] ~ dmvnorm2(0, sigmal_group, Rho_l_group),
        c(ap_group,bsocp_group)[group_index] ~ dmvnorm2(0, sigmap_group, Rho_p_group),
        c(al_dom,bsocl_dom)[domain_index] ~ dmvnorm2(0, sigmal_dom, Rho_l_dom),
        c(ap_dom,bsocp_dom)[domain_index] ~ dmvnorm2(0, sigmap_dom, Rho_p_dom),
        c(ap,al)~ dstudent(2,0,1),
...
mD.male <- map2stan(
    alist(
        ...
        al_id[mono_index] ~ dnorm(0, sigmal_id),
        ap_id[mono_index] ~ dnorm(0, sigmap_id),
        c(al_group,bmalel_group)[group_index] ~ dmvnorm2(0, sigmal_group, Rho_l_group),
        c(ap_group,bmalep_group)[group_index] ~ dmvnorm2(0, sigmap_group, Rho_p_group),
        c(al_dom,bmalel_dom)[domain_index] ~ dmvnorm2(0, sigmal_dom, Rho_l_dom),
        c(ap_dom,bmalep_dom)[domain_index] ~ dmvnorm2(0, sigmap_dom, Rho_p_dom),
        c(ap,al)~ dstudent(2,0,1),
        c(bmalep,bmalel)~ dnorm(0,1),
        c(sigmap_id,sigmal_id,sigmap_group,sigmal_group,sigmal_dom,sigmap_dom) ~ dexp(1),
        c(Rho_p_group, Rho_l_group, Rho_p_dom, Rho_l_dom) ~ dlkjcorr(3)
...
mD.rank <- map2stan(
    alist(
        ...
        al_id[mono_index] ~ dnorm(0, sigmal_id),
        ap_id[mono_index] ~ dnorm(0, sigmap_id),
        c(al_group,brankhil_group,branklol_group)[group_index] ~ dmvnorm2(0, sigmal_group, Rho_l_group),
        c(ap_group,brankhip_group,branklop_group)[group_index] ~ dmvnorm2(0, sigmap_group, Rho_p_group),
        c(al_dom,brankhil_dom,branklol_dom)[domain_index] ~ dmvnorm2(0, sigmal_dom, Rho_l_dom),
        c(ap_dom,brankhip_dom,branklop_dom)[domain_index] ~ dmvnorm2(0, sigmap_dom, Rho_p_dom),
        c(ap,al)~ dstudent(2,0,1),
        c(brankhip,brankhil,branklop,branklol,bsizep,bsizel)~ dnorm(0,1),
        c(sigmap_id,sigmal_id,sigmap_group,sigmal_group,sigmal_dom,sigmap_dom) ~ dexp(1),
        c(Rho_p_group, Rho_l_group, Rho_p_dom, Rho_l_dom) ~ dlkjcorr(3)
...
mD.rankG <- map2stan(
    alist(
        inno_count ~ dzipois( p , lambda ),
        logit(p) <- ap + logsampeff + ap_group + ap_dom + 
        (brankhip  + brankhip_group + brankhip_dom + bgrHp*groupsize)*rankhi +
        (branklop  + branklop_group + branklop_dom+ bgrLp*groupsize)*ranklo + bsizep*groupsize,

        log(lambda) <- al + logsampeff + al_group  + al_dom + 
        (brankhil + brankhil_group + brankhil_dom + bgrHl*groupsize)*rankhi +
        (branklol + branklol_group + branklol_dom + bgrLl*groupsize)*ranklo + bsizel*groupsize,
        ...
        c(ap_dom,brankhip_dom,branklop_dom)[domain_index] ~ dmvnorm2(0, sigmap_dom, Rho_p_dom),
        c(ap,al)~ dstudent(2,0,1),
        c(brankhip,brankhil,branklop,branklol,bsizep,bsizel,bgrHp,bgrHl,bgrLp,bgrLl)~ dnorm(0,1),
        c(sigmap_id,sigmal_id,sigmap_group,sigmal_group,sigmal_dom,sigmap_dom) ~ dexp(1),
        c(Rho_p_group, Rho_l_group, Rho_p_dom, Rho_l_dom) ~ dlkjcorr(3)
...
data=list(
    ...
    cores=2 , chains=2 , warmup=1000, iter=2000, WAIC=TRUE, types=list(adapt.delta=0.9)
)
...
,
        ...
        c(al_id,bsocl_id,bagel_id)[mono_index] ~ dmvnorm2(0, sigmal_id, Rho_l_id),
        c(ap_id,bsocp_id,bagep_id)[mono_index] ~ dmvnorm2(0, sigmap_id, Rho_p_id),
        c(al_group,bsocl_group,bagel_group,bmalel_group,brankhil_group,branklol_group)[group_index] ~ dmvnorm2(0, sigmal_group, Rho_l_group),
        c(ap_group,bsocp_group,bagep_group,bmalep_group,brankhip_group,branklop_group)[group_index] ~ dmvnorm2(0, sigmap_group, Rho_p_group),
        c(al_dom,bsocl_dom,bagel_dom,bmalel_dom,brankhil_dom,branklol_dom)[domain_index] ~ dmvnorm2(0, sigmal_dom, Rho_l_dom),
        c(ap_dom,bsocp_dom,bagep_dom,bmalep_dom,brankhip_dom,branklop_dom)[domain_index] ~ dmvnorm2(0, sigmap_dom, Rho_p_dom),
        c(ap,al)~ dstudent(2,0,1),
        c(bsocp,bsocl,bagep,bagel,bmalep,bmalel,brankhip,brankhil,branklop,branklol,bsizel,bsizep)~ dnorm(0,1),
        c(sigmap_id,sigmal_id,sigmap_group,sigmal_group,sigmal_dom,sigmap_dom) ~ dcauchy(0,1),
        c(Rho_p_id, Rho_l_id,Rho_p_group, Rho_l_group, Rho_p_dom, Rho_l_dom) ~ dlkjcorr(3)
...
compare(mD,mD.age,mD.soc,mD.rank,mD.male,mD.ageglobal)@output, "WAICtableinnovation.csv") #save WAIC outputs
write.csv(precis(mD.ageglobal, depth=2, digits=2  )@output, "globalmodeloutput.csv") #save model summary output for parameter estimates
...
par(mfrow = c(2,2))
par(cex = 0.4)
par(mar = c(2, 2, 2, 0), oma = c(3, 3, 0, 1))
for(i in 1:max(d$domain_index)){
        plot(inno_count~logage.c, data=d[d$domain_index==i,], xlab="log(age)", ylab="# of innovations per year",  
        col=col.alpha("white", alpha=0.05) , pch=19 , cex.lab=2.2 , ylim=c(0,2) , xaxt="n", cex.axis=0.8)
        title(domain_listp[i], line = -2 , cex.main=1.5)
...
link2 <- link(mD.ageglobal  , data=d.pred,replace=list(ap_id=id_zeros, al_id=id_zeros,
    ...
    branklop_dom=domain_zeros, brankhil_group=group_zeros,branklol_group=group_zeros,
    brankhip_group=group_zeros,branklop_group=group_zeros ), WAIC=TRUE)
    #pred.p.med <- apply((1-link2$p)  , 2 , mean)
    #pred.p.PI <- apply((1-link2$p)  , 2 , PI , prob=0.8)
    ...
    lines(pred.p.med ~ age.seq , lw=1, col=col.pal[i] , lty=1)

}
    mtext("individual innovation rate", outer =TRUE, cex = 1.2, side=2, line=1)
...
domain_list <- sort(unique(d$domain))
col.pal <- brewer.pal(4, "Dark2")
par(mfrow = c(2,2))
par(cex = 0.4)
par(mar = c(2, 2, 2, 0), oma = c(3, 3, 0, 1))
...
#link2$lambda*link2$p 
    pred.p.med <- apply(link2$lambda*(1-link2$p)  , 2 , median)
    pred.p.PI <- apply(link2$lambda*(1-link2$p)  , 2 , PI , prob=0.89)
    plot(inno_count~sociality.s, data=d[d$domain_index==i,], xlab="sociality scores (standardized)", ylab="# of innovations per year",  col=col.alpha("white", alpha=0.00005) , pch=19 , cex.lab=2 , ylim=c(0,2), cex.axis=0.8)
    title(domain_listp[i], line = -2 , cex.main=1.5)
...
link2 <- link(mD.ageglobal  , data=d.pred,replace=list(ap_id=id_zeros, al_id=id_zeros,
    ...
    #if(i==1){ mtext("# of innovations per year conditional", outer =FALSE, cex = .8, side=2, line=3)
             #mtext("on being an innovator", outer =FALSE, cex = .8, side=2, line=2)}
